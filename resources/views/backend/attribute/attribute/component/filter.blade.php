<div class="col">
    <div class="d-flex justify-content-between align-items-center mb-3">
        ...
        <div class="col-sm-auto">
            <div>
                <a href="{{ route('attribute.create') }}" class="btn btn-success add-btn"><i
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <form id="bulk-delete-form" action="{{ route('attribute.bulkdelete') }}" method="POST">
                        @csrf
                        @method('DELETE')
...
<form id="searchForm" action="{{ route('attribute.index') }}" method="GET">
    <div class="row g-4 mb-3 fz-12 text-muted">
        ...
        <div class="col-sm-auto">
            <div class="d-flex">
                <div class="fillter-attribute_catalogue_id ms-2">
                    <select name="attribute_catalogue_id" id="attribute_catalogue_id"
                        class="form-control text-muted setUpSelect2" style="width: 180px">
                        <option value="">[Nhóm thuộc tính]</option>
                        @if ($attributeCatalogues != null)
                            @foreach ($attributeCatalogues as $key => $catalogue)
                                <option value="{{ $catalogue->id }}" {{ old('attribute_catalogue_id', request('attribute_catalogue_id')) == $catalogue->id ? ' selected' : ''}}>{{ $catalogue->name }}</option>
                            @endforeach
                        @endif
                    </select>
                </div>
