<div class="main-content">
    <div class="page-content">
        <div class="container-fluid">
            ...
            <form action="{{ route('post.edit', ['slug' => $post->slug]) }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="row">
                    <div class="col-lg-8">
                        <div class="card">
                            <div class="card-body">
                                <div class="mb-3">
                                    ...
                                    <input type="text" class="form-control" name="name" id=""
                                        value="{{ old('name', $post->name) }}" placeholder="Tên bài viết">
                                    @if ($errors->has('name'))
                                ...
                                <div>
                                    <label>Mô tả</label>
                                    <div>
                                        <textarea class="form-control ck-editor" id="description" data-height="150" name="description">{{ old('description', $post->description) }}</textarea>
                                    </div>
                                </div>
                                <div class="mt-3 mb-2">
                                    <label>Nội dung</label>
                                    <div>
                                        <textarea class="form-control ck-editor " id="content" data-height="300" name="content">{{ old('content', $post->content) }}</textarea>
                                    ...
                                    <select class="form-select setUpSelect2" name="post_catalogue_id[]" multiple="multiple">
                                        ...
                                        @foreach ($postCatalogues as $key => $catalogue)
                                            <option value="{{ $catalogue->id }}"
                                                {{ in_array($catalogue->id, $post->postCatalogues->pluck('id')->toArray()) ? 'selected' : '' }}>
                            ...
                            <div class="card-body">
                                <div class="mb-3">
                                    ...
                                    <input type="text" name="cre" class="form-control" id=""
                                        value="{{ old('cre', $post->cre) }}" placeholder="cre bài viết">
                            ...
                            <div class="card-body">
                                <div class="mb-3">
                                    ...
                                    <select class="form-select setUpSelect2" name="publish">
                                        ...
                                        <option value="1" {{ $post->publish == 1 ? 'selected' : '' }}>Hiển thị
                                        </option>
                                        <option value="0" {{ $post->publish == 0 ? 'selected' : '' }}>Ẩn</option>
                                    </select>
                                    ...
                                    <div class="text-center">
                                        <div class="position-relative d-inline-block">
                                            ...
                                            <span class="image-target">
                                                <img src="{{ old('image', $post->image ?? '') ? ''. old('image', $post->image ?? '') : '/libaries/upload/images/img-notfound.png' }}"
                                                    alt=""
