<div class="main-content">
    <div class="page-content">
        <div class="container-fluid">
            ...
            <form action="{{ route('user.catalogue.edit', ['slug' => $userCatalogue->slug]) }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="row">
                    <div class="col-lg-8">
                        <div class="card">
                            <div class="card-body">
                                <div class="mb-3">
                                    ...
                                    <input type="text" class="form-control" name="name" id=""
                                        value="{{ old('name', $userCatalogue->name) }}" placeholder="Tên nhóm thành viên">
                                    @if ($errors->has('name'))
                                ...
                                <div class="tab-content">
                                    <div class="tab-pane active" id="addproduct-general-info" role="tabpanel">
                                        <div class="row">
                                            <div class="col-lg-8">
                                                <div class="mb-3">
                                                    <label class="form-label" for="manufacturer-name-input">Slug:<span
                                                            class="text-danger fz-18">*</span></label>
                                                    <input type="text" name="slug" class="form-control"
                                                        id="manufacturer-name-input" value="{{ old('slug', $userCatalogue->slug) }}"
                                            ...
                                            <div class="col-lg-4">
                                                <div class="">
                                                    <label class="form-label">Tên gọi tắt:<span
                                                            class="text-danger fz-18">*</span></label>
                                                    <input type="text" name="acronym" class="form-control"
                                                        value="{{ old('acronym', $userCatalogue->acronym) }}"
                                                        placeholder="Nhập tên gợi nhớ của nhóm">
                                                    @if ($errors->has('acronym'))
                                                        <span
                                                            class="text-danger fz-12 mt-1">{{ $errors->first('acronym') }}</span>
                                                    @endif
                                                </div>
                                            </div>
                                        </div>
                                ...
                                <div>
                                    <label>Mô tả</label>
                                    <div>
                                        <textarea class="form-control ck-editor" id="ck-editor" data-height="200" name="description">{{ $userCatalogue->description }}</textarea>
                                    </div>
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="card-body">
                                <div class="mb-3">
                                    ...
                                    <select class="form-select" name="publish">
                                        <option value="1" {{ ($userCatalogue->publish >= 1) ? 'selected' : '' }}>Hiển thị</option>
                                        <option value="0" {{ ($userCatalogue->publish == 0) ? 'selected' : '' }}>Ẩn</option>
                                    </select>
                                </div>
                            </div>
                        </div>
                                    ...
                                    <div class="text-center">
                                        <div class="position-relative d-inline-block">
                                            ...
                                            <span class="image-target">
                                                <img src="{{ old('image', $userCatalogue->image ?? '') ? '' . old('image', $userCatalogue->image ?? '') : '/libaries/upload/images/img-notfound.png' }}"
                                                    alt=""
                                                    class="render-image  object-fit-contain rounded-1 mb-2 position-relative "
                                                    width="96" height="96">
                                            </span>
                                            {{-- input ẩn gửi lên controller xử lý  --}}
                                            <input type="hidden" name="image" value="{{ $userCatalogue->image }}">
                                        </div>
                                    </div>
                                </div>
                            </div>
